...
EXPECTED_HASH="d9c57ca97d76062e0aa38da277c46dce2456b03ac08c336ebcb36fc348a1480c"
...
let CURRENT_VERSION = "v8.1-Beta-arm64"
...
struct AIChatView: View {
        ...
        ScrollView {
            VStack(spacing: 14) {
                ...
                Text("Privacy: messages are sent to \(chat.selectedProvider.rawValue) using your API key and saved on this Mac per provider. Do not paste passwords, tokens, or private files.")
                    .font(rNitroFont(9)).foregroundColor(.secondary).multilineTextAlignment(.center)
                    .padding(.horizontal, 20)
    ...
    private var chatPanel: some View {
        VStack(spacing: 0) {
            HStack(spacing: 6) {
                Text(chat.selectedProvider.rawValue).font(rNitroFont(compact ? 11 : 12, weight: .semibold))
            ...
            if !compact {
                Text("History is saved on this Mac. Messages also go to \(chat.selectedProvider.rawValue) — do not share secrets.")
                    .font(rNitroFont(8)).foregroundColor(.secondary)
...
<dict>
    ...
    <key>CFBundleVersion</key><string>8.1-Beta-arm64</string>
    <key>CFBundleShortVersionString</key><string>8.1-Beta-arm64</string>
