...
#
# v8.3.0-Beta-arm64 — iStats Menu-style UI: combined menubar slots, sectioned
# popover (CPU/GPU/Memory/Disk/Network/Battery/Sensors/Extras), load avg,
# memory pressure, disk activity, fan RPM readouts. Chat/BTC/stress kept in Extras.
#
...
EXPECTED_HASH="a59360da487902d1f8d717c97245d059c958219d9afdbd45dc85152166c771a8"
...
class DiskActivityMonitor: ObservableObject {
    ...
    private func sample() {
        guard let out = NetworkMonitor.runToolPublic("/usr/sbin/iostat", ["-d", "-n", "0", "-w", "1", "-c", "2"]) else { return }
        ...
        guard parts.count >= 3,
              let readKB = Double(parts[parts.count - 2]),
              let writeKB = Double(parts[parts.count - 1]) else { return }
        let read = readKB / 1024.0
        let write = writeKB / 1024.0
        DispatchQueue.main.async {
            self.readMBps = read
            self.writeMBps = write
            self.activityHistory.removeFirst()
            self.activityHistory.append(read + write)
        }
    }
}
...
enum MenuBarLayout: String, CaseIterable, Identifiable {
    ...
    var label: String {
        switch self {
        case .combined: return "Combined (stacked)"
...
enum MenuBarConfig {
    ...
    static var layout: MenuBarLayout {
        MenuBarLayout(rawValue: UserDefaults.standard.string(forKey: MonitorPreferences.menuBarLayoutKey) ?? "") ?? .combined
    }
...
enum MenuBarStatusFormatter {
    ...
    static func render(layout: MenuBarLayout) -> String {
        ...
        case .combined:
            let labels = slots.map(\.shortLabel).joined(separator: "  ")
            let values = slots.map { slotLabel($0) }.joined(separator: "  ")
            return labels.isEmpty ? "\(Int(cpu.totalUsage.rounded()))%" : "\(labels)\n\(values)"
        }
    }
}
...
struct ContentView: View {
    ...
    @AppStorage(MonitorPreferences.menuBarLayoutKey) private var menuBarLayoutRaw = MenuBarLayout.combined.rawValue
