...
# Per-provider Keychain keys. Fixed menu bar popover layout and stat popups.
#
# v6.2.4-Final-arm64 — Stable macOS build without AI chat (see rNitro-v6.2.4-Final-arm64.sh).
#
# v6.2.3-Final-arm64 — Chat tab in the main window (not the menu bar popover):
...
EXPECTED_HASH="6038123c9782e2ff1a45556f3ba46b173ed60f83b81a8714f012ffa73ae6c43d"
...
let CURRENT_VERSION = "v7.0.1-Beta-arm64"
...
enum AppTab: String, CaseIterable, Identifiable {
    ...
    case chat = "Chat"
    var id: String { rawValue }
    static let popoverTabs: [AppTab] = [.monitor, .game]
    static let windowTabs: [AppTab] = [.monitor, .game, .chat]
}
...
final class AIChatModel: ObservableObject {
    ...
    private init() {
        for p in AIProvider.allCases {
            if let k = AIKeychain.load(provider: p) { keys[p] = k }
        }
    ...
    func hasSavedKey(for provider: AIProvider) -> Bool {
        keys[provider] != nil || AIKeychain.hasKey(for: provider)
    }
    ...
    func saveApiKey() {
        ...
        AIKeychain.save(k, provider: selectedProvider)
        keys[selectedProvider] = k
        showKeyEditor = false
        apiKeyDraft = ""
    }
    ...
    func sendMessage() {
        ...
        guard let apiKey = keys[selectedProvider] ?? AIKeychain.load(provider: selectedProvider) else {
            showKeyEditor = true
            return
        }
    ...
    nonisolated private static func parseAPIError(data: Data, status: Int, domain: String) throws {
        if let err = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
            if let e = err["error"] as? [String: Any], let msg = e["message"] as? String {
                throw NSError(domain: domain, code: status, userInfo: [NSLocalizedDescriptionKey: msg])
            }
            if let msg = err["message"] as? String {
                throw NSError(domain: domain, code: status, userInfo: [NSLocalizedDescriptionKey: msg])
            }
        }
        throw NSError(domain: domain, code: status, userInfo: [NSLocalizedDescriptionKey: "HTTP \(status)"])
    }

    nonisolated private static func requestGemini(apiKey: String, messages: [ChatMessage]) async throws -> String {
        var url = URLComponents(string: "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent")!
...
struct AIChatView: View {
        ...
        HStack(spacing: 6) {
            ForEach(AIProvider.allCases) { p in
                Button(action: { chat.selectProvider(p) }) {
                    Text(p.rawValue)
                        .font(rNitroFont(9, weight: chat.selectedProvider == p ? .semibold : .regular))
    ...
    private var apiKeyPanel: some View {
        ScrollView {
            VStack(spacing: 14) {
                Text("AI Chat").font(rNitroFont(15, weight: .semibold))
                providerPicker
                Text("Paste your \(chat.selectedProvider.rawValue) API key. Stored in Keychain — only sent to \(chat.selectedProvider.rawValue) when you chat.")
                ...
                SecureField("API key", text: $chat.apiKeyDraft)
                    ...
                    .padding(.horizontal, 20)
                MinimalButton(title: "Save Key", action: { chat.saveApiKey() })
                Link("Get a key at \(chat.selectedProvider.keyHint)", destination: URL(string: chat.selectedProvider.keyURL)!)
                    .font(rNitroFont(9)).foregroundColor(.accent)
                if chat.currentHasKey {
                    Button("Cancel") { chat.showKeyEditor = false }
                        .font(rNitroFont(10)).foregroundColor(.secondary).buttonStyle(.plain)
                }
            }
            .padding(.vertical, 16)
        }
    ...
    private var chatPanel: some View {
        VStack(spacing: 0) {
            HStack {
                Text(chat.selectedProvider.rawValue).font(rNitroFont(12, weight: .semibold))
                Spacer()
                Button("Change key") { chat.showKeyEditor = true; chat.apiKeyDraft = "" }
                    .font(rNitroFont(9)).foregroundColor(.secondary).buttonStyle(.plain)
            }
            .padding(.horizontal, 14).padding(.vertical, 8)
            providerPicker.padding(.horizontal, 14).padding(.bottom, 8)
                    ...
                    LazyVStack(alignment: .leading, spacing: 10) {
                        if chat.messages.isEmpty {
                            Text("Ask anything — powered by \(chat.selectedProvider.modelLabel).")
                                .font(rNitroFont(10)).foregroundColor(.secondary)
                                .padding(.top, 8)
                        }
                        ...
                        if chat.isLoading {
                            HStack(spacing: 6) {
                                ...
                                Text("Thinking…").font(rNitroFont(9)).foregroundColor(.secondary)
                            }
                        }
                    }
                    .padding(14)
...
<dict>
    ...
    <key>CFBundleVersion</key><string>7.0.1-Beta-arm64</string>
    <key>CFBundleShortVersionString</key><string>7.0.1-Beta-arm64</string>
