...
# LM Studio, Ollama, and Hermes. Per-provider Keychain keys.
#
...
EXPECTED_HASH="70f8c996e8d2125ae69a051422483df39324b0ce303126916f8b416e248e3e10"
...
let CURRENT_VERSION = "v7.0.2-Beta-arm64"
...
final class AIChatModel: ObservableObject {
    ...
    private func resolvedKey(for provider: AIProvider) -> String? {
        let k = keys[provider] ?? AIKeychain.load(provider: provider)
        if provider.requiresApiKey { return k }
        return k ?? nil
    }
    ...
    func sendMessage() {
        ...
        if provider.requiresApiKey {
            guard let k = keys[provider] ?? AIKeychain.load(provider: provider), !k.isEmpty else {
                showKeyEditor = true
                return
            }
            apiKey = k
            ...
            apiKey = keys[provider] ?? AIKeychain.load(provider: provider) ?? "local"
        }
    ...
    nonisolated private static func requestOpenAICompatible(
        ...
        req.httpMethod = "POST"
        req.setValue("application/json", forHTTPHeaderField: "Content-Type")
        if !apiKey.isEmpty && apiKey != "local" {
            req.setValue("Bearer \(apiKey)", forHTTPHeaderField: "Authorization")
        }
    ...
    nonisolated private static func requestOpenRouter(apiKey: String, messages: [ChatMessage]) async throws -> String {
        try await requestOpenAICompatible(
            ...
            messages: messages,
            domain: "OpenRouter",
            extraHeaders: ["HTTP-Referer": "https://rnitro.netlify.app", "X-Title": "rNitro"]
        )
    }
            ...
            HStack(spacing: 6) {
                ForEach(AIProvider.allCases) { p in
                    Button(action: { chat.selectProvider(p) }) {
                        HStack(spacing: 4) {
                            Text(p.rawValue)
                            if chat.hasSavedKey(for: p) {
                                Circle().fill(Color.nGreen).frame(width: 4, height: 4)
                            }
                        }
...
<dict>
    ...
    <key>CFBundleVersion</key><string>7.0.2-Beta-arm64</string>
    <key>CFBundleShortVersionString</key><string>7.0.2-Beta-arm64</string>
