...
#
# v8.2.9-Final-arm64 — Stable release: CPU monitor + AI chat (OpenAI & OpenRouter only).
#
...
EXPECTED_HASH="6e37a91ba37be6a373c35b110f9c12d9c9c1e10413cea178e7d404a375cdcf68"
...
let CURRENT_VERSION = "v8.2.9-Final-arm64"
...
enum UpdateInstaller {
    ...
    private static func showDownloadProgress(for version: String) {
        ...
        let stack = NSStackView(frame: NSRect(x: 16, y: 16, width: 328, height: 88))
...
struct AppCleanerView: View {
    ...
    private func relativeLastUsed(_ date: Date?) -> String {
        if date == nil {
            return model.isEnrichingLastUsed ? "Last opened (macOS): checking…" : "Last opened (macOS): not recorded"
        }
        guard let date else { return "Last opened (macOS): not recorded" }
