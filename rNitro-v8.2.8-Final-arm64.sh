...
#
# v8.2.8-Final-arm64 — Stable release: CPU monitor + AI chat (OpenAI & OpenRouter only).
#
...
EXPECTED_HASH="bdaf64e448b307213d13e5e77ba800f1cf1f019a04e303bd509ee29fda186440"
...
let CURRENT_VERSION = "v8.2.8-Final-arm64"
    ...
    static func checkOnLaunch() {
        ...
        PinnedSession.shared.dataTask(with: req) { data, _, error in
            ...
            DispatchQueue.main.async {
                ...
                alert.informativeText = "You're running \(CURRENT_VERSION). A newer build is available: \(remote). Download and install now? rNitro will restart."
...
enum UpdateInstaller {
    static func zipURL(for version: String) -> URL {
        URL(string: "https://getrnitro.netlify.app/rNitro-\(version).zip")!
    }
    ...
    private static func performInstall(remoteVersion: String) -> InstallResult {
        ...
        let zipFile = tmp.appendingPathComponent("rNitro-update-\(remoteVersion).zip")
        ...
        var dlError: Error?
        var dlData: Data?
        var req = URLRequest(url: zipURL(for: remoteVersion))
        req.cachePolicy = .reloadIgnoringLocalCacheData
        ...
        sem.wait()
        guard dlError == nil, let data = dlData, !data.isEmpty else {
            return .failure("Could not download the update. Check your internet connection.")
        }
        do { try data.write(to: zipFile) } catch {
            return .failure("Could not save the update package.")
        }

        guard extractApp(from: zipFile, to: extractDir) != nil else {
            return .failure("Downloaded package could not be extracted.")
        }
        guard let staged = findAppBundle(in: extractDir) else {
            return .failure("rNitro.app not found inside the update package.")
        }

        applyUpdate(stagedApp: staged)
        return .success
    }

    private static func extractApp(from zipURL: URL, to destDir: URL) -> Bool {
        let fm = FileManager.default
    ...
    private static func applyUpdate(stagedApp: URL) {
        let dest = Bundle.main.bundlePath
        let scriptURL = FileManager.default.temporaryDirectory.appendingPathComponent("rnitro-apply-update.sh")
        let script = """
#!/bin/bash
sleep 1
rm -rf "\(dest)"
/usr/bin/ditto "\(stagedApp.path)" "\(dest)"
...
final class SystemAdvisorModel: ObservableObject {
    ...
    func answerQuery(_ raw: String) -> String {
        ...
        if matches(t, ["help", "what can", "how do"]) {
            ...
            Proactive warnings appear automatically when proactive alerts are enabled in ⚙ settings. Critical temperatures can also show macOS notification banners.
            """
        }
        return """
    ...
    private var settingsPanel: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 10) {
                ...
                .toggleStyle(.switch)
                .onChange(of: advisor.thresholds.criticalTempBannersEnabled) { advisor.refreshThresholds() }
            }
            .padding(compact ? 10 : 14)
        }
        .frame(maxHeight: compact ? 200 : 250)
...
struct AppCleanerView: View {
    ...
    var body: some View {
        ...
        .onChange(of: model.search) { _ in model.rebuildDisplayed() }
        .onChange(of: model.sort) { _ in model.rebuildDisplayed() }
        .onChange(of: model.leftoversReadyForPath) { path in
    ...
    private func relativeLastUsed(_ date: Date?) -> String {
        if date == nil {
            return model.isEnrichingLastUsed ? "Last used: checking…" : "Last used: not recorded"
        }
        guard let date else { return "Last used: not recorded" }
        ...
        fmt.timeStyle = .none
        return "Last used \(ago) · \(fmt.string(from: date))"
    }
...
struct ContentView: View {
    ...
    @AppStorage(MonitorPreferences.classicScrollKey) private var classicScrollMode = true
    ...
    var body: some View {
        MetricsReader(layout: layout) { _ in
            ...
            .onChange(of: showFirstLaunchTips) { showing in
...
<dict>
    ...
    <key>CFBundleVersion</key><string>8.2.8-Final-arm64</string>
    <key>CFBundleShortVersionString</key><string>8.2.8-Final-arm64</string>
