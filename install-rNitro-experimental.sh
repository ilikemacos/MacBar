...
echo ""
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/macbar-build.XXXXXXXX")"
...
func verifyBinaryIntegrity() {
    ...
    if SecStaticCodeCheckValidity(code, [], nil) != errSecSuccess {
        ...
        alert.informativeText = "The rNitro app signature is invalid — the bundle may have been modified after installation. Reinstall from getrnitro.netlify.app, or run: xattr -cr ~/Applications/MacBar.app"
...
enum UpdateInstaller {
    ...
    private static func findAppBundle(in dir: URL) -> URL? {
        ...
        for case let url as URL in e {
            ...
            if url.lastPathComponent == "MacBar.app" || url.lastPathComponent == "rNitro.app",
...
fileprivate final class IOHIDTempReader {
    ...
    private func fetchReadings() -> [Double] {
        // Private IOHID temp APIs trap on macOS 26 — skip them. SMC path still runs.
        return []
...
fileprivate final class IOReportPowerReader {
    ...
    private func setup() {
        ...
        for i in 0..<itemCount {
            guard let itemPtr = CFArrayGetValueAtIndex(allItems, i) else { continue }
    ...
    func sample() -> SocPowerSample? {
        // IOReport private APIs trap (EXC_BREAKPOINT) on macOS 26 utility threads.
        return nil
...
class BatteryMonitor: ObservableObject {
    ...
    private static func cfInt(_ value: CFTypeRef?) -> Int? {
        ...
        if CFGetTypeID(value) == CFNumberGetTypeID() {
            ...
            var v64 = Int64(0)
            let num = unsafeBitCast(value, to: CFNumber.self)
            if CFNumberGetValue(num, .sInt64Type, &v64) {
                return Int(v64)
            }
            var v32 = Int32(0)
            if CFNumberGetValue(num, .sInt32Type, &v32) {
                return Int(v32)
            }
        }
    ...
    private static func cfBool(_ value: CFTypeRef?) -> Bool? {
        ...
        if CFGetTypeID(value) == CFBooleanGetTypeID() { return CFBooleanGetValue(unsafeBitCast(value, to: CFBoolean.self)) }
...
enum ChopsticksAIKB {
    ...
    static let intents: [ChopsticksAIIntent] = [
        ...
        ChopsticksAIIntent(
            ...
            priority: 90,
            answer: "I'm cs.AI — Chopsticks HQ's assistant, built into MacBar's Chat tab.\n\nOnline it uses chopstickshq.com (live research, no API key from you). If you're offline it answers from a local knowledge base about MacBar, Fathom, and Chopsticks apps.\n\nAsk about MacBar (formerly rNitro), Fathom Air, Fathom Pro, installing, or privacy.",
        ...
        ChopsticksAIIntent(
            ...
            priority: 70,
            answer: "cs.AI live answers go to chopstickshq.com (same as the cs.AI Mac app). Offline fallback stays on your Mac.\n\nMacBar and Fathom send no product telemetry. Optional email accounts on cs.AI are only for chat sync and Pro — not required for MacBar sensors.",
...
final class AIChatModel: ObservableObject {
    ...
    nonisolated private static func requestChopsticks(messages: [ChatMessage], tier: String) async -> String {
        ...
        do {
            ...
            if let reply = obj["reply"] as? String, !reply.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                return reply
            }
...
func rNitroFont(_ role: FontRole, metrics: UIMetrics, weight: Font.Weight = .regular) -> Font {
    .system(size: metrics.size(role), weight: weight, design: .rounded)
}
        ...
        ScrollView {
            VStack(spacing: 0) {
                HStack(alignment: .top) {
                    VStack(alignment: .leading, spacing: 2) {
                        Text("MacBar").font(rNitroFont(.title, metrics: metrics, weight: .semibold))
                            .onTapGesture { RNitroEasterEgg.noteVersionTap() }
...
enum FontRegistrar {
    ...
    static func registerAll() {
        // Bundled Google Fonts + CTFontManager corrupt malloc on macOS 26.
        // UI uses system rounded fonts instead.
    }
...
enum AppLauncher {
    static func main() {
        NSSetUncaughtExceptionHandler { exception in
            NSLog("MacBar uncaught: %@ %@", exception.name.rawValue, exception.reason ?? "")
        }
...
if compile_one "arm64-apple-macos14.0" "$WORK_DIR/MacBar-arm64" \
  && compile_one "x86_64-apple-macos14.0" "$WORK_DIR/MacBar-x86_64" \
  && lipo -create -output "$WORK_DIR/MacBar" "$WORK_DIR/MacBar-arm64" "$WORK_DIR/MacBar-x86_64"; then
  echo "   universal binary (arm64 + x86_64), min macOS 14.0"
  rm -f "$WORK_DIR/MacBar-arm64" "$WORK_DIR/MacBar-x86_64"
elif [[ "$HOST_ARCH" == "arm64" ]]; then
  echo "   ⚠ universal build failed — arm64-only, min macOS 14.0"
  compile_one "arm64-apple-macos14.0" "$WORK_DIR/MacBar"
else
  echo "   ⚠ universal build failed — x86_64-only, min macOS 14.0"
  compile_one "x86_64-apple-macos14.0" "$WORK_DIR/MacBar"
fi
...
rm -rf -- "$APP_DEST"
rm -rf -- "$HOME/Applications/MacBar.app"
...
echo "🔤 Skipping bundled Google Fonts (they crash SwiftUI on macOS 26)."
