...
#
# v8.3.1-Final-arm64 — Stable release: CPU monitor + AI chat (OpenAI & OpenRouter only).
#
...
EXPECTED_HASH="0bf633bf879e9c435f783f26dedd2203c8a558015f5c3fa94249d2f4ea2acf97"
...
let CURRENT_VERSION = "v8.3.1-Final-arm64"
...
enum UpdateInstaller {
    ...
    private static func performInstall(remoteVersion: String) -> InstallResult {
        ...
        accumulator.version = remoteVersion
        let cfg = URLSessionConfiguration.ephemeral
        let session = URLSession(configuration: cfg, delegate: accumulator, delegateQueue: nil)
        ...
        guard dlError == nil, !data.isEmpty else {
            return .failure("Could not download the update. Check your internet connection.")
        }
        if accumulator.httpStatus != 0 && accumulator.httpStatus != 200 {
            return .failure("Server returned HTTP \(accumulator.httpStatus) for rNitro-\(remoteVersion).zip. The file may be missing — download manually from getrnitro.netlify.app.")
        }
        if data.count >= 2, data[0] == 0x3C, data[1] == 0x21 {
            return .failure("Downloaded an HTML page instead of the App ZIP (often a 404). Open the website and download rNitro-\(remoteVersion).zip manually.")
        }
        ...
        guard data.count >= 1_400_000 else {
            return .failure("Downloaded package is too small (\(data.count) bytes). Try again or use the website.")
        }
        do { try data.write(to: zipFile) } catch {
            return .failure("Could not save the update package.")
        }

        guard extractApp(from: zipFile, to: extractDir) else {
    ...
    private static func applyUpdate(stagedApp: URL) {
        ...
        let needsAdmin = destDir == "/Applications"
        ...
        let target = dest.replacingOccurrences(of: "'", with: "'\\''")
        let script: String
        if needsAdmin {
            script = """
#!/bin/bash
sleep 1
/usr/bin/osascript -e 'do shell script "rm -rf '\\''\(target)'\\'' && /usr/bin/ditto '\\''\(staged)'\\'' '\\''\(target)'\\'' && /usr/bin/xattr -cr '\\''\(target)'\\''" with administrator privileges' 2>/dev/null || exit 1
open '\(target)'
"""
        } else {
            script = """
...
final class HowItWorksModel: ObservableObject {
    @Published var stableId = "v8.3.1-Final-arm64"
...
<dict>
    ...
    <key>CFBundleVersion</key><string>8.3.1-Final-arm64</string>
    <key>CFBundleShortVersionString</key><string>8.3.1-Final-arm64</string>
