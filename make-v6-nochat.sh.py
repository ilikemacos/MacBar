...
STABLE_SUPPORTS_STREAMING = """    nonisolated private static func supportsStreaming(_ provider: AIProvider) -> Bool {
        switch provider {
...
STABLE_REQUEST_STREAMING_SWITCH = """        switch provider {
        case .openai:
...
def main() -> None:
    ...
    text = re.sub(
        r"nonisolated private static func supportsStreaming\(_ provider: AIProvider\) -> Bool \{\n        switch provider \{.*?\n        \}\n    \}",
        STABLE_SUPPORTS_STREAMING,
        text,
        count=1,
        flags=re.DOTALL,
    )
    ...
    text = text.replace('let RNITRO_BUILD_CHANNEL = "beta"', 'let RNITRO_BUILD_CHANNEL = "stable"')
