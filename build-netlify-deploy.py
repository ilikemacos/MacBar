...
def netlify_files(data: dict, *, include_bundle_zip: bool = False, website_zip: bool = False) -> list[str]:
    ...
    files = list(fr.WEBSITE_FILES) + [
        ...
        stable["sh"],
        beta["sh"],
        v.experimental_release(data).get("zip") or "",
        v.experimental_release(data).get("pkg") or "",
        v.experimental_release(data).get("dmg") or "",
        v.experimental_release(data).get("sh") or "",
        v.experimental_release(data).get("source_sh") or "install-rNitro-experimental.sh",
...
def stage(data: dict, *, include_bundle_zip: bool | None = None) -> Path:
    ...
    # Product UI lives on Chopsticks HQ; getrnitro keeps download/CDN URLs
    # (version.json, App ZIP/PKG/DMG/.sh, Netlify functions) for updaters + curl.
    hq = "https://chopstickshq.com/rnitro"
    netlify_toml = OUT_DIR / "netlify.toml"
    netlify_toml.write_text(
        "# getrnitro — download CDN + optional functions.\n"
        "# Browser marketing pages 301 → chopstickshq.com/rnitro\n"
        "[build]\n"
        "  publish = \".\"\n"
    ...
    # Mirror redirects for Netlify's _redirects file (same rules; CDN assets stay).
    _redirects = OUT_DIR / "_redirects"
    _redirects.write_text(
        f"/                 {hq}/                 301!\n"
        f"/index.html       {hq}/                 301!\n"
        f"/archives         {hq}/archives.html    301!\n"
        ...
        f"/terms            {hq}/terms.html       301!\n"
        f"/terms.html       {hq}/terms.html       301!\n"
        f"/cli              {hq}/cli.html         301!\n"
        f"/cli.html         {hq}/cli.html         301!\n"
        f"/linux            {hq}/linux.html       301!\n"
        f"/linux.html       {hq}/linux.html       301!\n"
        f"/windows          {hq}/windows.html     301!\n"
        f"/windows.html     {hq}/windows.html     301!\n"
        f"/404.html         {hq}/                 301!\n",
        encoding="utf-8",
    )
