...
def hero_dl_note(data: dict) -> str:
    ...
    gh = v.github_bundle_url(data)
    return f"""    <p style="color:var(--muted); font-size:15px; font-family:var(--mono); text-align:center; max-width:600px; margin-top:12px; line-height:1.55;">
      New here? Use the green <strong style="color:var(--green);">Recommended Download</strong> (App ZIP) above. Want everything in one file? <a href="{gh}" style="color:var(--cyan);">Download {bundle} on GitHub</a> (website + stable + beta + Windows). Expand <strong style="color:var(--text);">Other download options</strong> for PKG, DMG, or shell installers.
    </p>"""
