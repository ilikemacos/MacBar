...
#
# rNitro-Beta-0.1-Refresh-arm64 — Refresh channel: improved CLI, deeper AI integration (Advisor → Chat).
...
EXPECTED_HASH="dffa921577815c09d07c14fc884f880a81889d0ced85e763542360058c6c1b28"
...
let CURRENT_VERSION = "rNitro-Beta-0.1-Refresh-arm64"
...
final class AIChatModel: ObservableObject {
    ...
    func queueSystemAnalysis() {
        let cpu = CPUMonitor.shared
        let gpu = GPUMonitor.shared
        let bat = BatteryMonitor.shared
        let memTotal = cpu.memoryUsedGB + cpu.memoryFreeGB
        var lines = [
            "Analyze my Mac and suggest tuning steps:",
            "CPU \(String(format: "%.0f", min(100, cpu.totalUsage)))% · \(cpu.logicalCores) cores · \(String(format: "%.0f", cpu.temperature))°C",
            "GPU \(String(format: "%.0f", min(100, gpu.usage)))%",
            "RAM \(String(format: "%.1f", cpu.memoryUsedGB))/\(String(format: "%.1f", memTotal)) GB",
        ]
        if bat.isPresent {
            lines.append("Battery \(bat.levelPercent)% \(bat.isCharging ? "charging" : "on battery")")
        }
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                MinimalButton(title: "Open rNitro CLI", action: { CLIIntegration.openInTerminal() })
...
struct SystemAdvisorView: View {
    ...
    private var advisorHeader: some View {
        HStack(spacing: 8) {
            ...
            if !compact {
                Button("Ask AI") { openChatWithAnalysis() }
                    .font(rNitroFont(.caption, metrics: metrics, weight: .semibold)).foregroundColor(.accent).buttonStyle(.plain)
    ...
    private func openChatWithAnalysis() {
        AIChatModel.shared.queueSystemAnalysis()
        NotificationCenter.default.post(
            name: .rNitroOpenMainWindow,
            object: nil,
            userInfo: ["tab": AppTab.chat.rawValue, "chatSection": ChatSection.chat.rawValue]
        )
    }
...
enum CLIIntegration {
    static func openInTerminal() {
        ...
        tell application "Terminal"
            activate
            do script "export PATH=\\"$HOME/bin:$PATH\\"; rnitro"
        end tell
        """
