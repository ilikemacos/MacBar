...
EXPECTED_HASH="167d0d490ee8bf0699456ccdb06f1df24a89728ed59f1121afb495139416567c"
...
let CURRENT_VERSION = "v6.2.3-Final-arm64"
...
// ── Gemini chat (BYO API key, stored in Keychain) ───────────────────────────
...
enum GeminiKeychain {
    static let service = "app.rnitro.gemini"
...
@MainActor
final class GeminiChatModel: ObservableObject {
    static let shared = GeminiChatModel()
    ...
    func sendMessage() {
        ...
        guard !text.isEmpty, !apiKey.isEmpty, !isLoading else { return }
        inputText = ""
        ...
        Task {
            do {
                let reply = try await Self.requestGemini(apiKey: key, messages: history)
    ...
    nonisolated private static func requestGemini(apiKey: String, messages: [ChatMessage]) async throws -> String {
        ...
        let contents: [[String: Any]] = messages.filter { !$0.isError }.map { msg in
            [
        ...
        if http.statusCode != 200 {
            if let err = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
               let e = err["error"] as? [String: Any],
               let msg = e["message"] as? String {
                throw NSError(domain: "Gemini", code: http.statusCode, userInfo: [NSLocalizedDescriptionKey: msg])
            }
            throw NSError(domain: "Gemini", code: http.statusCode, userInfo: [NSLocalizedDescriptionKey: "HTTP \(http.statusCode)"])
        }
...
struct GeminiChatView: View {
    @ObservedObject private var chat = GeminiChatModel.shared

    var body: some View {
        Group {
            if !chat.hasSavedKey || chat.showKeyEditor {
                apiKeyPanel
            } else {
                chatPanel
            }
    ...
    private var apiKeyPanel: some View {
        VStack(spacing: 14) {
            Spacer(minLength: 8)
            Text("Gemini Chat").font(rNitroFont(15, weight: .semibold))
            Text("Paste your Google Gemini API key. It stays in your Mac Keychain and is only sent to Google when you chat.")
    ...
    private var chatPanel: some View {
        VStack(spacing: 0) {
            HStack {
                Text("Gemini").font(rNitroFont(12, weight: .semibold))
                Spacer()
                ...
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 10) {
                        if chat.messages.isEmpty {
                            Text("Ask anything — powered by gemini-2.0-flash.")
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {        UpdateChecker.checkOnLaunch()
        ...
        let hosting = NSHostingController(
            rootView: ContentView(tabs: AppTab.popoverTabs)
...
struct rNitroApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            ContentView(tabs: AppTab.windowTabs)
...
<dict>
    ...
    <key>CFBundleVersion</key><string>6.2.3-Final-arm64</string>
    <key>CFBundleShortVersionString</key><string>6.2.3-Final-arm64</string>
