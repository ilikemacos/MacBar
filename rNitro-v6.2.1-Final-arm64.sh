...
EXPECTED_HASH="6533b5de08a1a60c454f044bbf979c0db7ba8f80b2277cffd7675619e3c8475f"
...
let CURRENT_VERSION = "v6.2.1-Final-arm64"
...
class CPUMonitor: ObservableObject {
    ...
    private func updateDerived() {
        ...
        let smcTemp = smcReadings.isEmpty ? nil : smcReadings.reduce(0, +) / Double(smcReadings.count)
        let temp = smcTemp ?? CPUMonitor.thermalDisplayValue(state, usage: totalUsage)
        let boost = baseClock + (baseClock * 0.28) * (totalUsage / 100.0)
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
            self.thermalState = state
            self.tempSource = smcTemp != nil ? "Apple SMC (\(smcReadings.count) sensors)" : "macOS thermalState estimate"
    ...
    private var detailRows: [(String, String)] {
        ...
        case .temperature:
            return [
                ...
                ("Nominal Range", "35–50 °C at low load"),
                ("Fair Range", "55–70 °C under moderate load"),
...
<dict>
    ...
    <key>CFBundleVersion</key><string>6.2.1-Final-arm64</string>
    <key>CFBundleShortVersionString</key><string>6.2.1-Final-arm64</string>
