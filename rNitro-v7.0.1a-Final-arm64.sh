...
#
# v7.0.1a-Final-arm64 — Stable release: responsive CPU monitor, no AI chat.
#
...
EXPECTED_HASH="e16c96c853b3c6faf66e35227386a73b5ec8bb55a261ca0ddf5e4578fd4e2d4a"
...
let CURRENT_VERSION = "v7.0.1a-Final-arm64"
...
class CPUMonitor: ObservableObject {
    ...
    static func thermalDisplayValue(_ state: ProcessInfo.ThermalState, usage: Double) -> Double {
        ...
        switch state {
        case .nominal:  return 42 + 55 * u   // 42-97C
        case .fair:     return 58 + 47 * u   // 58-105C
        case .serious:  return 74 + 26 * u   // 74-100C
        case .critical: return 88 + 17 * u   // 88-105C
        @unknown default: return 42 + 55 * u
        }
    }
    ...
    static func resolveTemperature(state: ProcessInfo.ThermalState, usage: Double, smcReadings: [Double]) -> (temp: Double, source: String) {
        ...
        guard let peak = smcReadings.max(), !smcReadings.isEmpty else {
            return (estimate, "macOS thermalState + load estimate")
        }
        let lowPackageReading = peak < 62 && usage > 25
        let temp = lowPackageReading ? max(peak, estimate) : peak
        if lowPackageReading {
            return (temp, "Blended (\(smcReadings.count) sensors, peak \(Int(peak.rounded()))°C + load)")
        }
        return (temp, "Sensor peak (\(smcReadings.count) sensors)")
    }
    ...
    func startMonitoring() {
        let t = Timer.scheduledTimer(withTimeInterval: 0.75, repeats: true) { [weak self] _ in self?.update() }
    ...
    private func updateCPUUsage() {
        ...
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
            self.totalUsage = min(100, max(0, avg))
    ...
    private func updateDerived() {
        ...
        let socSample = IOReportPowerReader.shared.sample()
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
...
enum AppTab: String, CaseIterable, Identifiable {
    ...
    var id: String { rawValue }
    static let allTabs: [AppTab] = [.monitor]
}
    ...
    private var detailRows: [(String, String)] {
        ...
        case .temperature:
            return [
                ...
                ("Fair Range", "58–105 °C under moderate thermal pressure"),
                ("Serious/Critical", "74–105 °C — thermal throttling likely")
            ]
        case .cores:
...
enum MenuBarDisplayMode: String, CaseIterable, Identifiable {
    case full, cpuTemp, cpuOnly, cpuPower, battery, minimal

    var id: String { rawValue }

    var label: String {
        switch self {
        case .full: return "Full (₿ + CPU + Temp)"
    ...
    static var current: MenuBarDisplayMode {
        MenuBarDisplayMode(rawValue: UserDefaults.standard.string(forKey: MonitorPreferences.menuBarModeKey) ?? "") ?? .full
    }

    static func setCurrent(_ mode: MenuBarDisplayMode) {
        UserDefaults.standard.set(mode.rawValue, forKey: MonitorPreferences.menuBarModeKey)
...
enum MenuBarStatusFormatter {
    static func render(mode: MenuBarDisplayMode) -> String {
        let cpu = CPUMonitor.shared
        let bat = BatteryMonitor.shared
        let pct = Int(cpu.totalUsage.rounded())
        let temp = Int(cpu.temperature.rounded())
        let btcStr: String
        if let p = BTCPriceMonitor.shared.priceUSD {
            btcStr = String(format: "₿$%.1fk", p / 1000)
        } else {
            btcStr = "₿…"
        }
        switch mode {
        case .full:
            return "\(btcStr)  CPU: \(pct)%  Temp: \(temp)°"
        case .cpuTemp:
            return "CPU: \(pct)%  Temp: \(temp)°"
        case .cpuOnly:
            return "CPU: \(pct)%"
        case .cpuPower:
            return String(format: "CPU: %d%%  %.1fW", pct, cpu.packagePowerWatts)
        case .battery:
            guard bat.isPresent else { return "CPU: \(pct)%" }
            var text = "\(bat.levelPercent)%"
            if bat.isCharging {
                if let eta = bat.timeToFullMinutes, eta > 0, eta < 65535 {
                    text += eta >= 60 ? String(format: " · %dh %dm", eta / 60, eta % 60) : " · \(eta)m"
                } else {
                    text += " · charging"
                }
            } else if !bat.isOnAC, let rem = bat.remainingTimeText {
                text += " · \(rem)"
            } else if bat.isOnAC {
                text += " · AC"
            }
...
struct ContentView: View {
    ...
    @AppStorage(MonitorPreferences.menuBarModeKey) private var menuBarModeRaw = MenuBarDisplayMode.full.rawValue
        ...
        ScrollView {
            VStack(spacing: 0) {
                ...
                MinimalDivider().padding(.horizontal, 16)
                    ...
                    // Tools — menubar style, stress & benchmark
                    VStack(alignment: .leading, spacing: 10) {
                        Text("Tools").font(rNitroFont(.label, metrics: metrics)).foregroundColor(.secondary)
                        VStack(alignment: .leading, spacing: 6) {
                            Text("Menu Bar").font(rNitroFont(.caption, metrics: metrics)).foregroundColor(.secondary.opacity(0.85))
                            Picker("Menu Bar", selection: $menuBarModeRaw) {
                        ...
                        .toggleStyle(.switch)
                        .onChange(of: showStressUI) {
                            if !showStressUI { stress.stop() }
                        }
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {        UpdateChecker.checkOnLaunch()
        ...
        let popoverSize = NSSize(width: 400, height: 560)
        let popoverView = ContentView(tabs: AppTab.allTabs, layout: .popover)
    ...
    @objc private func togglePopover() {
        ...
        if event.type == .rightMouseUp {
            ...
            let current = MenuBarDisplayMode.current
            for mode in MenuBarDisplayMode.allCases {
                let item = NSMenuItem(title: mode.label, action: #selector(setMenuBarMode(_:)), keyEquivalent: "")
                item.target = self
    ...
    @objc private func setMenuBarMode(_ sender: NSMenuItem) {
        guard let raw = sender.representedObject as? String,
              let mode = MenuBarDisplayMode(rawValue: raw) else { return }
        MenuBarDisplayMode.setCurrent(mode)
        updateStatusTitle()
    }
    ...
    private func updateStatusTitle() {
        ...
        button.title = MenuBarStatusFormatter.render(mode: MenuBarDisplayMode.current)
    }
...
struct rNitroApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            ContentView(tabs: AppTab.allTabs, layout: .window)
...
<dict>
    ...
    <key>CFBundleVersion</key><string>7.0.1a-Final-arm64</string>
    <key>CFBundleShortVersionString</key><string>7.0.1a-Final-arm64</string>
